...
interface TestSuite {
  name: string;
  runner: { run: () => { passed: number; failed: number } | Promise<{ passed: number; failed: number }> };
...
class UnifiedTestRunner {
  ...
  private generateFinalReport(): void {
    ...
    const successRateNum = totalTests > 0 ? (totalPassed / totalTests * 100) : 0;
    ...
    if (totalTests > 0 && successRateNum < 95) {
